...
**Targeting Rule**: One condition in a `targeted` flag's `rules` array — an `attribute`/`operator`/`value` triple, where `operator` is one of `EQ`/`NEQ`/`IN`/`NOT_IN`/`CONTAINS`/`CUSTOM`. A rule set uses AND-only semantics — every rule must match, with no per-flag configurable operator ([ADR-0009](docs/adr/0009-targeting-rules-use-and-semantics.md)). OR semantics are expressed as separate flags, not a rule-set option. All evaluation happens client-side inside the SDK ([ADR-0005](docs/adr/0005-client-side-flag-evaluation.md)); the API only stores and ships rule contents, it never evaluates them.

**Custom Rule Handler**: The `CUSTOM` targeting-rule operator's mechanism for logic that doesn't fit `attribute`/`operator`/`value(s)`. The rule's `value` holds a single handler _name_ (a plain string, same shape as any other rule value) — never executable code. The SDK-integrating developer registers a same-named handler function locally in their own application via `SdkClient.registerCustomRule(name, handler)`; the dashboard/API never author, store, or ship any code for this, only the name. A handler receives the evaluation context and an app-supplied `deps` object (see **SDK client** construction), and may be sync or async. An unregistered name, or a handler that throws/rejects, evaluates that rule to `false` rather than failing the overall check.
_Avoid_: implying a `CUSTOM` rule ships or runs dashboard-authored code — it deliberately never does, to keep evaluation inside ADR-0005's client-side, zero-server-round-trip model without introducing remote code execution into every connected SDK client.
