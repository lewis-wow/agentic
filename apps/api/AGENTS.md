...
- **All Prisma queries live here.** No other app or package may query the database directly. `apps/dashboard` is a narrow exception: it may import the `prisma` singleton solely to construct a DI service class (`packages/bff`'s `UserService`, `packages/api`'s `ProjectService`) — see [ADR-0023](../../docs/adr/0023-dashboard-prisma-access-via-di-services.md) — never to query it itself.
