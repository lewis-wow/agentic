...
### The one sanctioned exception: DI services for Trusted Proxy Authentication and pre-project bootstrap

A small, fixed set of files exist purely to resolve the Trusted Proxy Authentication headers into the identity used for page gating, or to bootstrap the installation before any project exists — this **is** the "authorize the user" mechanism for server-rendered pages, not business-data access. This is no longer a raw-Prisma exception: per [ADR-0023](../../docs/adr/0023-dashboard-prisma-access-via-di-services.md), these files never call `prisma.*` themselves — they construct and call the DI service classes documented there (`packages/bff`'s `UserService`, `packages/api`'s `ProjectService`), importing the `prisma` singleton only to pass into a service constructor:

- `src/lib/guards.ts` — `resolveAuthedUser()` (the underlying resolution: validates the Trusted Proxy Secret + Identity Header via `@repo/bff`'s `resolveTrustedProxyUser`, JIT-provisioning the `User` via `@repo/bff`'s `UserService.upsert()` — an upsert with an empty `update` clause so an existing user's role is never overwritten) plus `requireSession()` / `requireOwner()` / `requireProjectAccess()` built on top. Used by server components purely to gate rendering (`unauthorized()`, `forbidden()`) and to read `projectRole` for conditional UI (`canManage`) — never to fetch data that gets rendered. Note this app never mints or verifies a JWT itself — that's `apps/bff`'s job for the actual data path; this is a separate, UX-level gate. `guards.ts` also exports `projectsExist()` (`@repo/api/services`' `ProjectService.exists()`, `cache()`-wrapped) for the pre-project-existence checks below.
- `src/app/(public)/setup/**` — the pre-auth bootstrap flow. `setup/page.tsx` calls `projectsExist()` to decide whether setup is still needed; `setup/actions.ts`'s server action calls `ProjectService.exists()` again as an "already completed" guard against a race/replay, then `ProjectService.createWithEnvironments({ name, environmentNames: ['development', 'production'] })` to atomically create the first project once the designated owner — `TRUSTED_PROXY_OWNER_EMAIL` — has been seen. This runs before any project exists, so treat it as a narrow, separate exception from ordinary business-data CRUD, not a precedent to extend elsewhere.
- `src/app/page.tsx`, `src/app/(dashboard)/layout.tsx` — call `guards.ts`'s `projectsExist()` to redirect to `/setup` on a fresh install.
