...
### The one sanctioned Prisma exception: Trusted Proxy Authentication and pre-project bootstrap

A small, fixed set of files exist purely to resolve the Trusted Proxy Authentication headers into the identity used for page gating, or to bootstrap the installation before any project exists — this **is** the "authorize the user" mechanism for server-rendered pages, not business-data access. Per [ADR-0023](../../docs/adr/0023-dashboard-prisma-access-via-di-services.md), these files never call `prisma.*` themselves; they construct and call DI service classes from `packages/bff`/`packages/api`, importing the `prisma` singleton only to pass into the service's constructor:
...
- `src/app/(public)/setup/**` — the pre-auth bootstrap flow (checking whether any project exists yet via `guards.ts`'s `projectsExist()`, creating the first project once the designated owner — `TRUSTED_PROXY_OWNER_EMAIL` — has been seen). This runs before any project exists, so treat it as a narrow, separate exception from ordinary business-data CRUD, not a precedent to extend elsewhere.
- `src/app/page.tsx`, `src/app/(dashboard)/layout.tsx` — call `guards.ts`'s `projectsExist()` (`@repo/api/services`' `ProjectService.exists()`, `cache()`-wrapped so a layout and its page don't each run the query) to redirect to `/setup` on a fresh install.
