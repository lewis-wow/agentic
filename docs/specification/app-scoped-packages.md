...
`apps/api` keeps only what's inherently tied to Hono: route handlers, the JWT middleware, the Hono-specific half of request validation (`src/validation.ts`, which wires a schema to `@hono/standard-validator`'s `sValidator`), and app-specific runtime infrastructure that a service shouldn't own directly (e.g. the in-memory SSE event emitter in `src/events/`). A service that needs that kind of infrastructure takes it through its `Options` constructor param, typed as a local structural interface — never by importing the concrete `apps/api` module, which would invert the dependency direction. See `packages/api/AGENTS.md` and `docs/specification/effect-schema.md`.
