...
- **Ticket PR**: once a ticket is done (checklist green, `/code-review` findings addressed), push the ticket branch and open a PR from it into the *spec branch* (not `master`). Comment on the ticket issue linking the PR (not a bare commit, since there's no master commit yet), same `CLAUDE: ` prefix convention. Do not merge it yourself — that's the user's review/merge action.
